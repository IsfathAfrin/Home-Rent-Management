...
$nameErr = $passwordErr= $ConfirmPasswordErr=$emailErr=$phonenoErr=$areaErr=$flatErr=$rentErr= "";
$name = $password =$ConfirmPassword =$email=$phoneno=$area=$flat=$rent= "";
...
if ($_POST) {
  ...
  if(!empty($_POST["name"]) &&!empty($_POST["phoneno"])&& !empty($_POST["password"])&& !empty($_POST["ConfirmPassword"])&& !empty($_POST["rent"])&& !empty($_POST["flat"])&& !empty($_POST["email"])&& (($_POST["password"])==($_POST["ConfirmPassword"])))
  {
	...
	$ownerinfo = "INSERT INTO ownerinfo(name, email,phoneno,password,flatnumbers,rentamount)
            VALUES ('$Name','$Email','$Phoneno','$Password','$flat','$rent')";
